...
class SmartFactoryPublisher:
    ...
    def publish_reading(self, line: str, sensor: str) -> SensorReading:
        ...
        qos_level = SENSORS[sensor]["qos"]
        self._client.publish(self._topic(line, sensor), json_str, qos=qos_level)
        log.info(f"[{line}/{sensor}] value={reading.value} {reading.unit}  QoS={qos_level}  seq={reading.seq}")
        return reading
